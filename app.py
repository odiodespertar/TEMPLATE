...
<div style="background-color: #25282b; color: white; padding: 12px; border-radius: 6px; font-weight: bold; text-align: center; margin-bottom: 10px; border: 1px solid #3b3f43; display: flex; flex-direction: column; align-items: center; gap: 4px;">
    ...
    <div id="nombre-ruteo-activo-header" style="font-size: 21px; color: #FFFFFF; text-transform: uppercase; font-weight: 800; letter-spacing: 1px;">
        C1 SCP1
    </div>
</div>
    ...
    <div id="fleet-drag-handle" style="display: flex; justify-content: center; align-items: center; gap: 8px; flex-wrap: wrap; padding: 4px 0; margin-bottom: 8px;">
		...
		<button id="excel-btn" onclick="toggleExcelView()" style="cursor:pointer; background: linear-gradient(180deg, #327332 0%, #5a8f32 100%); color: white; border: 1px solid #008000; font-size: 12px; padding: 4px 9px; border-radius: 6px; font-weight: bold; outline: none;">
            📊 VISTA EXCEL
        </button>
    </div>
   ...
   function crearTabYContenidoEnPantalla(nombreRuteo, flotaElegida, planesElegidos, idBD = null, incluirORH = false, incluirOcup = false, llevaNodos = false) {{
        ...
        let botonesAccion = idBD ? `
            <div style="display: flex; gap: 8px; justify-content: flex-end; margin-bottom: 8px;">
                <button onclick="guardarCambiosRuteoActual(${{nuevoTabId}}, ${{idBD}})" style="cursor:pointer; background: #6495ED; color: white; border: none; padding: 4px 10px; font-size: 11px; font-weight: bold; border-radius: 4px;">💾 GUARDAR CAMBIOS</button>
            </div>
        ` : '';
...
<div id="panel-mapa-operativo" style="display: none; margin-top: 10px; padding: 10px; background: #17191b; border: 1px solid #34383d; border-radius: 12px; text-align: center;">
    ...
    <!-- Contenedor con overflow para permitir Scroll cuando haya Zoom -->
    <div id="contenedor-img-scroll" style="overflow: auto; max-height: 400px; border-radius: 8px; border: 1px solid #222;">
        <img 
            id="img-mapa-operativo"
            src="https://drive.google.com/thumbnail?id=1M4GLEwFzhLrZjV-zmvGrdTQhC6IjwxOJ&sz=w1000" 
            alt="Mapa Operativo" 
            onclick="abrirMapaPantallaCompleta()"
            style="width: 100%; transition: transform 0.2s ease; transform-origin: top left; cursor: zoom-in;" 
            title="Haz clic para abrir en pantalla completa"
        />
    </div>
</div>

<!-- MODAL / PANTALLA COMPLETA SUPERPUESTA -->
    ...
    <div style="margin-bottom: 10px; background: #202326; border: 1px solid #3a3f44; border-radius: 8px; overflow: hidden;">
        ...
        <div id="box-mensajes-bot" style="max-height: 480px; overflow-y: auto; display: flex; flex-direction: column; gap: 10px; line-height: 1.6; font-size: 16px; padding: 6px;">
            <div style="background: #262626; border-left: 3px solid #008218; padding: 8px; border-radius: 6px; color: #ffffff;">
                🤖 <b>Asistente de Ruteo:</b><br>Consulta el SVC o presiona arriba para cierre de ruteo.
            </div>
        </div>

        <div style="display: flex; gap: 6px; margin-top: 10px;">
            <input type="text" id="input-bot-lateral" placeholder="Escribe tu consulta..." onkeydown="if(event.key==='Enter') enviarConsultaBotLateral()" style="flex: 1; padding: 8px 10px; border-radius: 6px; border: 1px solid #444; background: #141414; color: white; font-size: 16px;">
            <button onclick="enviarConsultaBotLateral()" style="cursor: pointer; background: #20B2AA; color: white; border: none; padding: 8px 12px; border-radius: 6px; font-weight: bold; font-size: 14px;">🚀</button>
        </div>
    </div>
</div>
...
<script>
   ...
   function enviarConsultaBotLateral(opcionDirecta = null) {{
        ...
        box.innerHTML += `
            <div style="background: #008080; border-right: 3px solid #66CDAA; padding: 8px; border-radius: 6px; color: #ffffff; text-align: right; margin-bottom: 6px;">
                <b>Tú:</b> ${{consulta}}
            </div>
        `;
    ...
    function generarReporteFinalResumen() {{
        ...
        let resumenFinal = 
            ...
            `📌 Se cargaron las Rentals como híbridas en Plan local, pero el sistema no las consideró todas como híbridas.<br>` +
            `${{textoUnidades}}<br>` +
            `${{textoBulk}}` +
            `${{textoDropeo}}<br>` +
            `${{textoPlanND}}` +
    ...
    function aplicarZoomMapa(factor) {{
        ...
        // Limitar el zoom mínimo a 1x y el máximo a 3x
